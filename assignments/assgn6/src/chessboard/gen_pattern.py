...
class PatternMaker:
        ...
        for x in range(0, self.cols):
            for y in range(0, self.rows):
                if x % 2 == y % 2:
                    square = SVG(
                        "rect",
                        x=x * spacing + xspacing,
                        y=y * spacing + yspacing,
                        width=spacing,
                        height=spacing,
                        fill="black",
                        stroke="none",
        ...
        for i in range(n):
            if corners[i] == "right":
                res_str += "L{},{} L{},{} ".format(
                    x + cw_point[i][0],
                    y + cw_point[i][1],
                    x + mid_cw_point[(i + 1) % n][0],
                    y + mid_cw_point[(i + 1) % n][1],
                )
            elif corners[i] == "round":
                res_str += "A{},{} 0,0,1 {},{} ".format(
                    rad,
                    rad,
                    x + mid_cw_point[(i + 1) % n][0],
                    y + mid_cw_point[(i + 1) % n][1],
    ...
    def save(self):
        c = canvas(
            self.g,
            width="%d%s" % (self.width, self.units),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="generate camera-calibration pattern", add_help=False
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-s",
        "--square_size",
        help="size of squares in pattern",
        default="20.0",
        action="store",
        dest="square_size",
        type=float,
    ...
    parser.add_argument(
        "-R",
        "--radius_rate",
        help="circles_radius = square_size/radius_rate",
        default="5.0",
        action="store",
        dest="radius_rate",
        type=float,
    ...
    parser.add_argument(
        "-a",
        "--page_size",
        help="page size, superseded if -h and -w are set",
        default="A4",
        action="store",
        dest="page_size",
    ...
    parser.add_argument(
        "-m",
        "--markers",
        help="list of cells with markers for the radon checkerboard. Marker "
    ...
    if p_type == "radon_checkerboard" and "markers" in args:
        if len(args.markers) % 2 == 1:
            raise ValueError(
                "The length of the markers array={} must be even".format(
                    len(args.markers)
                )
            )
        markers = set()
        for x, y in zip(args.markers[::2], args.markers[1::2]):
            if x in range(0, columns) and y in range(0, rows):
                ...
                raise ValueError(
                    "The marker {},{} is outside the checkerboard".format(x, y)
                )
