...
def integrate_frankot(zx, zy):
    ...
    Wx = np.tile(
        ...
        * np.pi
        / H2
        * np.hstack((np.arange(0, H2 / 2 + 1), np.arange(-H2 / 2 + 1, 0))).reshape(
            -1, 1
        ),
        (W2, 1),
    ).flatten()
    Wy = np.kron(
        ...
        * np.pi
        / W2
        * np.hstack((np.arange(0, W2 / 2 + 1), np.arange(-W2 / 2 + 1, 0))).reshape(
            -1, 1
        ),
        np.ones((H2, 1)),
    ).flatten()
...
def read_tif(filename, downsample=1):
    im = cv2.imread(filename, cv2.IMREAD_UNCHANGED)  # read as 16-bit
    cv2.cvtColor(im, cv2.COLOR_BGR2RGB, im)  # convert to RGB
    return im[::downsample, ::downsample] / (2**16 - 1)  # downsample and normalize
...
def gamma_encoding(im):
    ...
    apply gamma encoding to input images
        input: linear image
        output: non-linear image
    """
    encoded = np.where(
        im <= 0.0031308, im * 12.92, (1 + 0.055) * im ** (1 / 2.4) - 0.055
    )
    return encoded
...
def illuminant_xyz(illuminant_in):
    ...
    xy = np.array(
        [
            [0.44757, 0.40745, 0.45117, 0.40594],
...
def XYZ_to_RGB_linear(rgb_space="sRGB"):
    ...
    S = np.linalg.inv(np.array([[Xr, Xg, Xb], [Yr, Yg, Yb], [Zr, Zg, Zb]])) @ np.array(
        [Xw, Yw, Zw]
    )
